...
class UserProfile(Base):
    ...
    media_folders = relationship(
        "MediaFolder", back_populates="user", cascade="all, delete-orphan"
    )
    media_collections = relationship(
        "MediaCollection", back_populates="user", cascade="all, delete-orphan"
    )
...
class Asset(Base):
    ...
    playlist_items = relationship(
        "PlaylistItem", back_populates="asset", cascade="all, delete-orphan"
    )
    folder_links = relationship(
        "AssetFolderLink", back_populates="asset", cascade="all, delete-orphan"
    )
    collection_items = relationship(
        "CollectionItem", back_populates="asset", cascade="all, delete-orphan"
    )
...
class PlaylistItem(Base):
    ...
    __table_args__ = (CheckConstraint("position >= 0", name="check_position_positive"),)
...
class Stream(Base):
    ...
    playlist_id = Column(
        UUID(as_uuid=True),
        ForeignKey("playlists.id", ondelete="RESTRICT"),
        nullable=True,
    )
    ...
    video_collection_id = Column(
        UUID(as_uuid=True), ForeignKey("media_collections.id", ondelete="SET NULL")
    )
    audio_collection_id = Column(
        UUID(as_uuid=True), ForeignKey("media_collections.id", ondelete="SET NULL")
    )
    ...
    scheduled_start_enabled = Column(
    ...
    stream_destinations = relationship(
        "StreamDestination", back_populates="stream", cascade="all, delete-orphan"
    )
    stream_assets = relationship(
        "StreamAsset",
        back_populates="stream",
        cascade="all, delete-orphan",
        order_by="StreamAsset.position",
    )
    events = relationship(
        "StreamEvent", back_populates="stream", cascade="all, delete-orphan"
    )
    video_collection = relationship(
        "MediaCollection", foreign_keys=[video_collection_id]
    )
    audio_collection = relationship(
        "MediaCollection", foreign_keys=[audio_collection_id]
    )

    __table_args__ = (
        CheckConstraint(
            "status IN ('stopped', 'starting', 'running', 'error', 'stopping', 'scheduled')",
            name="check_status",
        ),
        CheckConstraint(
            "source_type IN ('playlist', 'assets')", name="check_source_type"
        ),
        CheckConstraint(
            "mix_mode IN ('video_only', 'audio_only', 'mixed')",
            name="check_stream_mix_mode",
        ),
        CheckConstraint(
            "schedule_repeat IN ('none', 'daily', 'weekly')",
            name="check_stream_schedule_repeat",
        ),
    )
...
class StreamEvent(Base):
    ...
    event_metadata = Column(
        "metadata", JSONB
    )  # 'metadata' reserved in SQLAlchemy, use event_metadata
    ...
    __table_args__ = (
        CheckConstraint(
            "level IN ('info', 'warning', 'error', 'debug')", name="check_level"
        ),
    )
...
class MediaFolder(Base):
    ...
    asset_links = relationship(
        "AssetFolderLink", back_populates="folder", cascade="all, delete-orphan"
    )
...
class AssetFolderLink(Base):
    ...
    __table_args__ = (PrimaryKeyConstraint("asset_id", "folder_id"),)
...
class MediaCollection(Base):
    ...
    collection_type = Column(
        Text, nullable=False, default="video_background", index=True
    )
    description = Column(Text)
    origin_playlist_id = Column(
        UUID(as_uuid=True), ForeignKey("playlists.id", ondelete="SET NULL"), unique=True
    )
...
class CollectionItem(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "loop_mode IN ('loop', 'once', 'shuffle')",
            name="check_collection_items_loop_mode",
        ),
    )
...
class AdminAction(Base):
    ...
    admin_user = relationship(
        "UserProfile", foreign_keys=[admin_user_id], back_populates="admin_actions"
    )
    target_user = relationship(
        "UserProfile",
        foreign_keys=[target_user_id],
        back_populates="target_admin_actions",
    )
...
class SystemAlert(Base):
    ...
    stream_id = Column(
        UUID(as_uuid=True), ForeignKey("streams.id", ondelete="CASCADE"), index=True
    )
    asset_id = Column(
    ...
    resolved_by_user = relationship(
        "UserProfile", foreign_keys=[resolved_by], back_populates="resolved_alerts"
    )
