...
# prctl(2) — second arg for PR_SET_DUMPABLE; the ``ctypes`` shim below is
# Linux-only. On macOS/BSD the call silently falls through; container
# production runs on Linux so this is the load-bearing platform.
_PR_SET_DUMPABLE = 4
_PR_SET_PDEATHSIG = 1
...
class FFmpegStreamManager:
        ...
        try:
            ...
            process = await asyncio.create_subprocess_exec(
                *cmd,
                ...
                preexec_fn=_harden_ffmpeg_child,
            )
            ...
            runtime_signal_state = (
                runtime_signals.normalize_runtime_signal_state(
            ...
            # Restart counter source-of-truth:
            #  - On a manual start (restart=False) the counter is reset to 0
            #    in memory and persisted to the DB so a previously-flapping
            #    stream gets a clean slate.
            #  - On an auto-restart (restart=True), prefer the in-memory
            #    counter if present; otherwise fall back to the persisted
            #    value (cleanup_dead_streams may have pruned the in-memory
            #    entry between failure and restart).
            if restart:
                in_memory = int(existing_info.get("restart_attempts", 0) or 0)
                persisted = await self._load_persisted_restart_attempts(stream_id)
            ...
            self.stream_info[stream_id] = {
                ...
                "restart_attempts": effective_attempts,
    ...
    async def _record_runtime_log_health(self, stream_id: str, line: str) -> None:
        ...
        if _line_matches_markers(
            normalized_line, runtime_signals.REMOTE_OUTPUT_RESET_MARKERS
        ):
        ...
        if _line_matches_markers(
            normalized_line, runtime_signals.RECOVERY_SUCCESS_MARKERS
        ):
        ...
        if _line_matches_markers(
            normalized_line, runtime_signals.NON_MONOTONIC_DTS_MARKERS
        ):
            await self._track_runtime_signal(
                stream_id,
                info,
    ...
    def _register_runtime_signal_hit(
        ...
        return runtime_signals.register_runtime_signal_hit(
            runtime_state,
            signal_name,
            log_line,
            observed_at,
    ...
    def _count_recent_signal_hits(
        ...
        *,
        ...
        return runtime_signals.count_recent_signal_hits(
            signal_state,
            now=now,
    ...
    def _signal_cooldown_elapsed(
        ...
        *,
        ...
        return runtime_signals.signal_cooldown_elapsed(
            signal_state,
            now=now,
    ...
    async def _track_runtime_signal(
        ...
        spec = runtime_signals.DEGRADED_SIGNAL_SPECS[signal_name]
    ...
    def get_runtime_incident_summary(self, stream_id: str) -> Dict[str, Any]:
        info = self.stream_info.get(stream_id)
        if not info:
            return runtime_signals.healthy_runtime_incident_summary()

        return runtime_signals.summarize_runtime_incidents(
    ...
    async def _handle_stream_failure(self, stream_id: str, returncode: int):
        ...
        # Pull the persistent counter from the DB and take the max of in-memory
        # and persisted values. This defends against the bug where
        # cleanup_dead_streams prunes stream_info between failure and restart,
    ...
    async def _fetch_daily_usage(self, user_id: UUID) -> Optional[Dict[str, Any]]:
        return await process_support.fetch_daily_usage(user_id)
    ...
    async def _load_persisted_restart_attempts(self, stream_id: str) -> int:
        ...
        try:
            async with get_db_context() as session:
                ...
                if stream is None:
                    return 0
                return int(getattr(stream, "runtime_restart_attempts", 0) or 0)
        except SQLAlchemyError as exc:
            logger.warning(
                "Could not read persisted restart_attempts for stream %s: %s",
                stream_id,
                exc,
            )
            return 0
            ...
            logger.exception(
                "Unexpected error reading persisted restart_attempts for stream %s",
                stream_id,
            )
            return 0
    ...
    def _rotate_log_file(log_file: Path, max_backups: int) -> None:
        process_support.rotate_log_file(log_file, max_backups)
