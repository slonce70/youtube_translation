...
- **Panel UX unification** (branch `redesign/unify-panel`): the new Mission Control overview and the existing `/dashboard/streaming` opened with two different compositions (health banner+tiles vs stat-strip+tabs). Unified the health design language without a rebuild (plan from a multi-agent analysis workflow):
  - Extracted the overview's health primitives into a shared `components/dashboard/stream-health.tsx` (`HealthState`, `STATE_COLOR`, `STATE_RANK`, `healthOf`, `HealthPill`, new `GlobalHealthBanner`); refactored `OverviewPage` to import them (zero behavior change, verified identical).
  - Streaming page: added the same `GlobalHealthBanner` as the lead element under the header (drives off the existing `degradedLiveEntries`/`runningStreams` so it never disagrees with the stat-strip), and added consistent `HealthPill`s to the Scheduled rows and a new "Статус" column in the Archive table.
  - Kept the stat-strip (capacity/limits) + `LiveStreamHero`/`StatusStrip` untouched (bold detail surface). Reused `dashboard.overview.*` labels; one new key `streaming.page.archive.tableStatus` across uk/en/ru.
  - Gates: `i18n:check`, `lint`, `type-check`, `jest` (160), `next build` — all green. Browser-verified overview (identical) + streaming (banner + Archive status pill).
